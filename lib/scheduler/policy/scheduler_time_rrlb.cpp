...
void scheduler_time_rrlb::compute_ue_ul_priorities(slot_point               pdcch_slot,
...
void scheduler_time_rrlb::save_dl_newtx_grants(span<const dl_msg_alloc> dl_grants)
{
  if (dl_grants.empty()) {
    return;
  }
...
void scheduler_time_rrlb::save_ul_newtx_grants(span<const ul_sched_info> ul_grants)
{
  if (ul_grants.empty()) {
    return;
  }
