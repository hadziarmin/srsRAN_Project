...
void scheduler_time_qos_lb::rem_ue(du_ue_index_t ue_index)
{
  ue_history_db.erase(ue_index);
}

void scheduler_time_qos_lb::compute_ue_dl_priorities(slot_point               pdcch_slot,
...
void scheduler_time_qos_lb::compute_ue_ul_priorities(slot_point               pdcch_slot,
...
void scheduler_time_qos_lb::save_dl_newtx_grants(span<const dl_msg_alloc> dl_grants)
{
...
void scheduler_time_qos_lb::save_ul_newtx_grants(span<const ul_sched_info> ul_grants)
{
...
scheduler_time_qos_lb::ue_ctxt::ue_ctxt(du_ue_index_t             ue_index_,
                                     du_cell_index_t           cell_index_,
                                     const scheduler_time_qos_lb* parent_) :
  ue_index(ue_index_),
...
void scheduler_time_qos_lb::ue_ctxt::compute_dl_prio(const slice_ue& u,
...
void scheduler_time_qos_lb::ue_ctxt::compute_ul_prio(const slice_ue& u,
                                                  slot_point      pdcch_slot,
                                                  slot_point      pusch_slot,
                                                  unsigned        nof_slots_elapsed)
{
...
void scheduler_time_qos_lb::ue_ctxt::compute_dl_avg_rate(const slice_ue& u, unsigned nof_slots_elapsed)
{
...
void scheduler_time_qos_lb::ue_ctxt::compute_ul_avg_rate(const slice_ue& u, unsigned nof_slots_elapsed)
{
  // In case more than one slot elapsed.
  if (nof_slots_elapsed > 1) {
...
void scheduler_time_qos_lb::ue_ctxt::save_dl_alloc(uint32_t total_alloc_bytes, const dl_msg_tb_info& tb_info)
{
  dl_sum_alloc_bytes += total_alloc_bytes;
}

void scheduler_time_qos_lb::ue_ctxt::save_ul_alloc(unsigned alloc_bytes)
{
  if (alloc_bytes == 0) {
    return;
  }
